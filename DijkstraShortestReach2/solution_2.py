...
def update_graph(source_graph, x, y, r):
	x -= 1
	y -= 1
	source_graph[x][y] = min(source_graph[x][y], r)
	source_graph[y][x] = min(source_graph[y][x], r)
...
"""
graph = NxN matrix
visited_set := nodes visited
path_list := [ [[n1, n2, . . . , ], total_distance], . . . ]
"""
...
def extend_path(graph, path_list, visited_set, N, end):
	...
	for possible_node in range(N):
		if graph[node - 1][possible_node] < 100001 and (possible_node + 1) not in visited_set:
